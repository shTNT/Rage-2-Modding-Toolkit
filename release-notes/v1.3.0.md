...
- **Most complete filelist to date shipped in-repo.** `filelist/`
  contains a bit-a-bit verified filelist with 36,192 unique hashes,
...
Every entry in `filelist/filelist.txt` has been re-hashed
from its path (`MurmurHash3 x64 128-bit`, seed=0, low 64 bits of h1)
and confirmed to exist in a game `.tab` file. See
`filelist/verification.txt`.

## Methods

`filelist/methods.txt` documents every method that produced
...
- Additional test vectors in `filelist/methods.txt`
